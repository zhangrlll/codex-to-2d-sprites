...
最终检查同时包含视觉检查、连续播放和文件验证。

| 检查方式 | 检查内容 |
| --- | --- |
| 逐帧检查 | 姿势与朝向、错肢、附件丢失、穿模、裁切、头部比例、颜色和尺寸抖动。 |
| 连续播放 | 动作节奏、脚底接触、相邻帧连贯性、循环首尾、区段连接和不合理停顿。 |
| 文件验证 | 动作／方向／帧数齐全，文件可读，alpha 正确，画布与锚点一致，图集每格对应正确独立帧。 |
| 交付验证 | 时序与循环标记正确，源文件可重新打开并播放，贴图齐全，ZIP 完整。 |

发现问题后，定位到绑定、动作、抽帧或绘制阶段，只返工受影响部分。图片看起来像棋盘背景不代表透明，必须检查实际 alpha。无法直接得到真透明时，可按工具规则及用户授权使用纯色背景，再去底、对齐和拼图；底色需要避开角色本身的颜色。

检查记录会保留。重复修复无效时应换方法，无法继续时明确说明具体缺口。图像生成存在波动，检查通过不代表绝对逐像素一致，尚存的发丝、衣褶等差异需要如实说明。
...
检查向右奔跑时的头部大小变化，以及跳跃落地后的停顿。
只修改受影响的部分，复检后更新素材包和检查记录。
```
